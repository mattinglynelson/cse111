...
def main():
    ...
    grams = float(input('How many grams of this substance are we dealing with? '))
    ...
    symbol_list = parse_formula(formula, table)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    # compute_molar_mass(symbol_qauntitity_list, periodic_table_dict)

    # Compute the number of moles in the sample.
    molar_mass = compute_molar_mass(symbol_list, table)[0]
    total_atomic_mass = compute_molar_mass(symbol_list, table)[1]
    # print (f'Atomic Mass = {total_atomic_mass}')
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    # Separate the list into symbol and quantity.
        # periodic_table_dict[key - Element Symbol][NAME_INDEX]
        # (elements[SYMBOL_INDEX], elements[QUANTITY_INDEX])
    total_molar_mass = 0
    total_atomic_mass = 0
    for elements in symbol_quantity_list:
        symbol = elements[SYMBOL_INDEX]
        quantity = elements[QUANTITY_INDEX]
    ...
    return total_molar_mass, total_atomic_mass
